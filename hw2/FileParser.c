...
char* ReadFileToString(const char* file_name, int* size) {
  ...
  if (S_ISREG(file_stat.st_mode) == 0) {
    fprintf(stderr, "given file is not a regular file");
    return NULL;
  }
  ...
  while (i <= size) {
      ...
      if (isalpha(cur_ptr[0])) {
          *cur_ptr = tolower(*cur_ptr);
          ...
          char temp = cur_ptr[0];
          cur_ptr[0] = '\0';
          int word_size = strlen(word_start);
          if (word_size > 0) {  // check for empty words
            AddWordPosition(tab, word_start, i - word_size);
          }
          cur_ptr[0] = temp;
          cur_ptr++;
          word_start = cur_ptr;
...
static void AddWordPosition(HashTable* tab, char* word,
  ...
  if (HashTable_Find(tab, hash_key, &kv)) {
    ...
    HashTable_Insert(tab, kv, &unused);
  }
}
