...
LinkedList* MemIndex_Search(MemIndex* index, char* query[], int query_len) {
  ...
  wp = kv.value;
  HTIterator* hti =  HTIterator_Allocate(wp->postings);
  Verify333(hti != NULL);
  bool has_next = true;
  HTKeyValue_t kvwp;
  while (has_next) {
    HTIterator_Get(hti, &kvwp);
    SearchResult* sr = (SearchResult*) malloc (sizeof(SearchResult));
    sr->doc_id = kvwp.key;
    sr->rank = LinkedList_NumElements(kvwp.value);
    LinkedList_Append(ret_list, sr);
    has_next = HTIterator_Next(hti);
  }
  HTIterator_Free(hti);
  ...
  for (i = 1; i < query_len; i++) {
    ...
    for (j = 0; j < num_docs; j++) {
      ...
      wp = kv.value;
      HTKeyValue_t okv;
      if (!HashTable_Find(wp->postings, doc_id, &okv)) {
        LLIterator_Remove(ll_it, (LLPayloadFreeFnPtr)free);
      } else {
        sr->rank += LinkedList_NumElements(okv.value);
