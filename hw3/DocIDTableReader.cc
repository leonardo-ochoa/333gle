  ...
  for (IndexFileOffset_t& curr_element : elements) {
    ...
    if (curr_header.doc_id == doc_id) {
      ...
      for (int i = 0; i < curr_header.num_positions; i++) {
        Verify333(fseek(file_, curr_element + sizeof(DocIDElementHeader)
                        + i * sizeof(DocIDElementPosition), SEEK_SET) == 0);
        Verify333(fread(&position, sizeof(DocIDElementPosition),
                        1, file_) == 1);
