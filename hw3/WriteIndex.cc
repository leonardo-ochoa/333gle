...
int WriteIndex(MemIndex* mi, DocTable* dt, const char* file_name) {
  ...
  int ih_bytes = WriteHeader(f, dt_bytes, mi_bytes);
  if (ih_bytes == kFailedWrite) {
    fclose(f);
    unlink(file_name);
    return kFailedWrite;
  }
...
static int WriteHeader(FILE* f, int doctable_bytes, int memidx_bytes) {
  ...
  while (left_to_read > 0) {
    size_t result = fread(buf, 1, kBufSize, f);
    if (result < 1) {
      return kFailedWrite;
    }
...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    ...
    int32_t check_chain = LinkedList_NumElements(ll);
    size = WriteHTBucketRecord(f, record_pos, check_chain, bucket_pos);
    if (size < 0) {
      return kFailedWrite;
    }
    record_pos += size;
    if (check_chain > 0) {
      size = WriteHTBucket(f, bucket_pos, ll, fn);
      if (size < 0) {
        return kFailedWrite;
      }
      bucket_pos += size;
...
static int WriteHTBucketRecord(FILE* f, IndexFileOffset_t offset,
  ...
  BucketRecord buck_rec(num_elts, bucket_offset);
  buck_rec.ToDiskFormat();
  ...
  if (fwrite(&buck_rec, sizeof(BucketRecord), 1, f) != 1) {
    return kFailedWrite;
  }
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    LLIterator_Get(it, reinterpret_cast<void**>(&kv));

    // STEP 8.
    // Write the element itself, using fn.
    int size = fn(f, element_pos, kv);
    if (size < 0) {
      LLIterator_Free(it);
      return kFailedWrite;
    }
    ...
    element_pos += size;  // you probably want to change this
    LLIterator_Next(it);
  }
  LLIterator_Free(it);
...
static int WriteDocIDToPositionListFn(FILE* f,
  ...
  for (int i = 0; i < num_positions; i++) {
    ...
    uintptr_t intptr = reinterpret_cast<uintptr_t>(payload);
    int32_t payload_converted = static_cast<int32_t>(intptr);
    // STEP 14.
    // Truncate to 32 bits, then convert it to network order and write it out.
    position.position = payload_converted;
