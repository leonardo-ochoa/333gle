...
// // Funtion that takes in a pointer and the length of
// // the pointer, it  prints out the length,
// // address passed, and its bytes of memory
// void DumpBytes(void* pData, int32_t byteLen);

// // function that takes in two arrays and thier length
// // funtion sorts contents of first array and copies
// // it into the second array
// void CopyAndSort(uint8_t arr1[], uint8_t arr2[], int32_t len);
...
//   DumpBytes(&int_val, sizeof(int_val));
//   DumpBytes(&float_val, sizeof(float_val));
//   DumpBytes(arr_unsorted, sizeof(uint8_t) * 11);
//   CopyAndSort(arr_unsorted, arr_sorted, 11);
//   DumpBytes(arr_sorted, sizeof(uint8_t) * 11);
...
//   uint8_t* arr = (uint8_t*) pData;
//   for (int i = 0; i < byteLen; i++) {
//     uint8_t a_byte = arr[i];
