...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  // STEP 1
  char check_root[PATH_MAX];
  char check_file[PATH_MAX];
  realpath(root_dir.c_str(), check_root);
  realpath(test_file.c_str(), check_file);
  if (check_root == nullptr || check_file == nullptr) {
    return false;
  }
  check_root[strlen(check_root) + 1] = '\0';
  check_root[strlen(check_root)] = '/';
  if (strncmp(check_root, check_file, strlen(check_root)) != 0) {
    return false;
  }
