...
static void HttpServer_ThrFn(ThreadPool::Task* t) {
  ...
  while (!done) {
    HttpRequest htreq;
    bool request = hc.GetNextRequest(&htreq);
    if (!request) {
      close(hst->client_fd);
      break;
    }

    HttpResponse rep = ProcessRequest(htreq, hst->base_dir, *hst->indices);
    bool response = hc.WriteResponse(rep);
    if (!response) {
      close(hst->client_fd);
      break;
    }
    ...
    string connection = htreq.GetHeaderValue("connection");
    if (connection == "close") {
        close(hst->client_fd);
        break;
    }
...
static HttpResponse ProcessFileRequest(const string& uri,
  ...
  if (file_reader.ReadFile(&contents)) {
    ...
    size_t shift = file_name.find('.');
    string suffix = file_name.substr(shift);
...
static HttpResponse ProcessQueryRequest(const string& uri,
  ...
  URLParser url_parser;
  url_parser.Parse(uri);
  map<string, string> arg = url_parser.args();
  string query = boost::to_lower_copy(arg["terms"]);
  vector<string> queries;
  boost::split(queries, query, boost::is_any_of(" "),
                boost::token_compress_on);
  hw3::QueryProcessor qp(indices, false);
  vector<hw3::QueryProcessor::QueryResult> result = qp.ProcessQuery(queries);
  if (result.size() != 0) {
    ret.AppendToBody("<p><br>\n");
    string size = std::to_string(result.size());
    ret.AppendToBody(size + " results found for ");
    ret.AppendToBody("<b>" + EscapeHtml(query) + "</b>\n");
    ret.AppendToBody("</p><p>\n");
    ret.AppendToBody("</p><ul>\n");
    for (auto& qr : result) {
      string path = qr.document_name;
      if (qr.document_name.find("http://") == std::string::npos) {
        path = "/static/" + qr.document_name;
      }
      ret.AppendToBody("<li>");
      ret.AppendToBody("<a href=\"" + path + "\">" + qr.document_name +
                        "</a> ");
      ret.AppendToBody("[" + std::to_string(qr.rank) + "]" + "<br>");
      ret.AppendToBody("</li>\n");
    }
    ret.AppendToBody("</ul>\n");
  } else {
